...
GND     ------> GND
VDD     ------> VCC
SCL		------> STM32 SPI1 SCK(PA5)
SDA		------> STM32 SPI1 MOSI(PA7)
RST		------> VCC (NO HARDWARE RESET)
DC		------> STM32 GPIO (PB0)
CS		------> STM32 GPIO (PB1)
BLK		------> VCC
*/
...
void ST7735_Draw_Char_8X16_by_ethan(int16_t x, int16_t y, char c, int16_t textColor, int16_t bgColor)
{
	...
	if(x+7 >= ST7735_WIDTH || y+15 >= ST7735_HEIGHT){
		return ;
	}

	ST7735_FillRectangle(x, y, 8, 16, bgColor);

	for(i = 0; i < 16; i++){
		//line = fontdata_8x16[(c*16)+i];
		line = g_font8X16[((c-32)*16)+i];
		...
		for(j = 0; j < 8; j++){
			if(line&(0x1 << j)){
				ST7735_DrawPixel(x+i%8, y+j+(i > 7? 8 : 0), textColor);
			}
		}
	}
...
void ST7735_Draw_String_8X16_by_ethan(int16_t x, int16_t y, char *str, int len, int16_t textColor, int16_t bgColor)
{
	...
	for(i = 0; i < len; i++){
		...
		ST7735_Draw_Char_8X16_by_ethan(x, y, str[i], textColor, bgColor);
		x += 8;
	}
	return ;
}
...
void ST7735_Draw_Symbol_8X16_by_ethan(int16_t x, int16_t y, const uint8_t *font, int16_t textColor, int16_t bgColor)
{
	...
	for(i = 0; i < 16; i++){
		...
		for(j = 0; j < 8; j++){
			if(line&(0x1 << j)){
				ST7735_DrawPixel(x+7-j, y+i, textColor);
			}
		}
	}
...
{
	ST7735_Draw_Symbol_8X16_by_ethan(x, y, g_specific_symbol_f8X16, textColor, bgColor);
	ST7735_Draw_Char_8X16_by_ethan(x+8, y, 'C', textColor, bgColor);
}
...
void ST7735_Draw_Chinese_16X16_by_ethan(int16_t x, int16_t y, const uint8_t *font, int chinese_index, int16_t textColor, int16_t bgColor)
{
	...
	uint8_t *ptr = NULL;
	ptr = (uint8_t *)font;
	ptr += chinese_index * 32;

	if(x+15 >= ST7735_WIDTH || y+15 >= ST7735_HEIGHT){
		return ;
	}

	ST7735_FillRectangle(x, y, 16, 16, bgColor);
	...
	for(i = 0; i < 16; i++){
		...
		for(j = 0; j < 8; j++){
			if(line&(0x1 << j)){
				ST7735_DrawPixel(x-j+7, y+i, textColor);
			}
		}
	}
	...
	for(i = 0; i < 16; i++){
		...
		for(j = 0; j < 8; j++){
			if(line&(0x1 << j)){
				ST7735_DrawPixel(x-j+15, y+i%16, textColor);
			}
		}
	}
...
void ST7735_DrawImage_by_ethan(uint16_t x_s, uint16_t y_s, uint16_t x_e, uint16_t y_e, const uint8_t* data, uint16_t len) {
  ...
  if(x_s >= ST7735_WIDTH || y_s >= ST7735_HEIGHT || x_e >= ST7735_WIDTH || y_e >= ST7735_HEIGHT){
    return;
  }

  LCD_CS0;  //Select

  ST7735_SetAddressWindow(x_s, y_s, x_e, y_e);

  for(i = 0; i < len; i++){
	  lcd7735_sendData(data[i]);
  }
  LCD_CS1;  //Unselect
}
...
void lcd7735_senddata(uint8_t data) {
  HAL_SPI_Transmit(&hspi1, &data, 1, 0x10);
}
